class SVGManipulator {
  ...
  onWheel(event) {
    ...
    if (
      newWidth <= this.zoom.max * this.cachedViewBox.width &&
      newHeight <= this.zoom.max * this.cachedViewBox.height &&
      newWidth >= this.zoom.min * this.cachedViewBox.width &&
      newHeight >= this.zoom.min * this.cachedViewBox.height
    ) {
      const fromVars = {
        ease: this.zoom.ease,
        x: this.viewBox.x,
  ...
  selectDraggable(event) {
    ...
    gsap.set(this.proxy, {
      x: this.pannable.pointerX,
      y: this.pannable.pointerY,
    });
